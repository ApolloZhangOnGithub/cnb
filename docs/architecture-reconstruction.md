...
- CLI 入口: `bin/cnb`, `bin/cnb.js`, `bin/board`, `bin/swarm`, `bin/init`, `bin/dispatcher`, `bin/doctor`, `bin/hygiene`, `bin/notify`, `bin/registry`, `bin/shutdown`.
...
- 侧车工具: `tools/cnb-mac-companion`, `tools/cnb-island.shit`, `tools/web-capture.shit`, `tools/cnb-sync-gateway`.
- 测试面: `tests/test_*.py`.
- 当前工作区状态: `git status --short`, `bin/hygiene --json`.

当前 worktree 有大量已修改、删除、未跟踪和被标记为备份的文件。因此本文以“当前磁盘状态”为准, 同时在“技术债”章节标出会影响复现的分裂点。
...
`lib/cli.py` 会尝试找到相邻的 `bin/cnb`, 找不到时 fallback 到 `/opt/homebrew/bin/cnb` 或 PATH 中的 `cnb`。

现状注意: Python wheel 当前只包含 `lib*`, 不包含 `bin/`, `schema.sql`, `migrations/`。如果从 pip wheel 复现, 必须先修 packaging contract 或改成纯 Python 入口。源码 checkout 和 npm tarball 是更接近当前真实运行路径的分发形态。
...
1. Pick lead engine from `CNB_AGENT`, `SWARM_AGENT`, `cnb codex`, or `--agent`.
2. If project exists, read sessions from config.
3. If fresh, choose session names from built-in themes, run `bin/init`, then start worker sessions.
4. Start dispatcher in background if needed.
5. Generate `.claude/commands/cnb-*.md` slash commands.
6. Launch the lead session:
...
Notification push exists as a concern module, with `notification_log` dedup and `notifications.toml`, but it is not currently in the dispatcher concern list. Re-enabling it requires adding it to `bin/dispatcher`.
...
- `lib.concerns.digest_scheduler` sends scheduled digests.
...
Current tracked path `tools/cnb-island/` is deleted in this dirty worktree, while a backup-like `tools/cnb-island.shit/` exists. The architecture documented by that tree:
...
Build scripts in that backup tree:
...
Current tracked `tools/web-capture/README.md` is deleted, while `tools/web-capture.shit/README.md` describes the sidecar boundary. Browser collectors should remain separate from cnb and call `cnb capture ingest`.
...
- Packaging/entrypoints: `bin/cnb`, `lib.cli`, npm package smoke helpers.
...
4. `bin/init` has source-tree side effects.
   - Project init can update source `registry/pubkeys.json` through `lib.board_mailbox`.
   - Reproduction from a checkout can dirty the source repo.
...
8. Current worktree contains many generated/local/backup-like files.
   - `bin/hygiene --json` reported generated/cache, local runtime state, marked backups, tracked deletions and untracked work.
...
5. Dispatcher:
   - `DispatcherConfig`.
   - `Concern` base.
   - health, idle, nudge, resource and time concerns.
...
lib/
  ...
  notification_*.py           notification config/delivery
...
tools/                        sidecar runbooks and companion apps
