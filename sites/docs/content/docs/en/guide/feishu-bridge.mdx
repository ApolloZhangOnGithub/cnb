---
title: Feishu Bridge
description: "Connect your Feishu chat to the Mac-level device supervisor for async command routing."
---
...
<Warning>Hermes / `hermes_lark_cli` is only a development test adapter. Do not put it in the production path.</Warning>
...
- Device chief and multi-device supervisors defines how multiple Mac device
  supervisors use Feishu as a human-visible control room without becoming
  split-brain active writers.
- Mac companion and Island explains the Mac companion and optional iPhone Live
  Activity bridge.
- `apps/mac/` embeds the same built-in Web TUI locally through
  WebKit.
- `apps/island/` can export Feishu chat config for simulator and physical
