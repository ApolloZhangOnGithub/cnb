...
**cnb 的方向：** 先定组织架构，再做自动化。当前首要任务是明确角色（终端主管同学 vs 项目负责同学、基础设施 ownership），把治理机制建进工具链，让合规成为默认路径而非自觉行为。之后让模块负责人完全自治——自动发现相关 issue、用 CI 验证自己的工作、创建 PR、响应故障。不是"无人值守的 agent 做随机任务"，而是"负责任的 owner 不需要人催着做份内的事"。详见 [ROADMAP.md](ROADMAP.md)。
...
| **终端主管同学** | 面向用户的 Claude Code 会话。在任意 Claude Code 会话中用 `/cnb` 激活。Per-machine（不是 per-project），管理这台机器上所有 cnb 项目。激活后所有操作被追踪、ownership 被匹配、组织有感知。 |
...
依赖：Python 3.11+、tmux，以及至少一个 agent CLI：Claude Code CLI（默认）或 Codex CLI。
...
/cnb            # 激活终端主管模式 — cnb 治理上线
```

注册为终端主管同学，激活操作追踪和 ownership 匹配的 hook，显示项目概览。终端主管同学是 per-machine 的，管理所有 cnb 项目。
...
初始化项目（创建 `.cnb/` 目录，含 SQLite 数据库和配置），在 tmux 中启动一组同学，启动调度器，然后进入终端主管同学的会话。
...
终端主管同学直接与用户对话。后台同学独立工作，通过看板汇报进展。

**飞书唤醒路径：** 在 `~/.cnb/config.toml` 里配置允许访问的飞书会话，然后在 Mac 上运行 `cnb feishu start`。来自该会话的飞书 IM 事件会被转进本机终端主管同学的 tmux 会话。

```toml
[feishu]
transport = "hermes_lark_cli"  # 开发测试通道：复用 Hermes 的飞书 CLI 机器人
chat_id = "oc_xxxxx"
...
启动前可用 `cnb feishu status` 检查 bridge 配置。
这是异步消息 bridge，不是终端画面镜像：Claude Code 的实时 TUI 渲染不会同步到飞书。转进来的消息会包含飞书 `message_id`；终端主管同学需要用 `cnb feishu reply <message_id> "消息内容"` 把用户可见进展发回飞书。

当前飞书通道明确标记为 `hermes_lark_cli`：它复用已有 Hermes 飞书 CLI 机器人做开发测试。不要把这个机器人当作 CNB 的生产身份，也不要把它当成 CNB 组织关系的事实来源。

飞书侧命令统一放在 CNB 命名空间，避免和通用命令冲突：

- `/cnb_tui` 或 `/c_tui`：回复当前终端主管 TUI 快照。
- `/cnb_watch` 或 `/c_watch`：启动内置只读 Web TUI 并回复链接。如果要发外网/隧道链接，配置 `watch_public_url`。
...
- **SQLite (WAL 模式)** — 所有状态存在 `.cnb/board.db`，每个项目一个数据库
...
目前还不行。终端主管同学需要人来驱动。但这是活跃开发方向——见 [ROADMAP.md](ROADMAP.md)。目标是让模块负责人能自主发现 issue、验证自己的工作、创建 PR、响应故障，不需要人催。
