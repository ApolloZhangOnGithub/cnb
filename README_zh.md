...
| 分发 | 公开 npm 包 `c-n-b`，内部为 Python 3.11+ |
...
标准公开包是 npmjs.com 上的 [`c-n-b`](https://www.npmjs.com/package/c-n-b)。GitHub Packages 也可能显示 scoped 镜像 `@apollozhangongithub/cnb`；npmjs 仍然是受支持的安装路径。发布和可见性规则见 [Package publishing](docs/package-publishing.md)。

要安装 npm 包名 `c-n-b`，它提供的命令才叫 `cnb`。不要运行 `npm install -g cnb`：npm 上的 `cnb` 是另一个无关包。如果 rename release 发布前 `npm install -g c-n-b` 还是 404，请等正式 release，不要临时改装 `cnb`。
