...
struct FeishuChatClient {
    ...
    func fetchRecentMessages(settings: FeishuChatSettings, limit: Int = 24) async throws -> [FeishuRemoteMessage] {
        try await fetchMessages(settings: settings, limit: limit).messages
    }
    ...
    func fetchMessages(
        ...
        limit: Int = 30,
        pageToken: String = ""
    ) async throws -> FeishuMessagePage {
        let token = try await tenantAccessToken(settings: settings)
        var queryItems = [
        ...
        if !pageToken.trimmed.isEmpty {
            queryItems.append(URLQueryItem(name: "page_token", value: pageToken.trimmed))
        }
        ...
        let items: [FeishuMessageItem] = data?.items ?? []
        let messages: [FeishuRemoteMessage] = items.compactMap { item in
        ...
        return FeishuMessagePage(
            ...
            hasMore: data?.hasMore == true && !nextPageToken.isEmpty
        )
    }
    ...
    private func notifyBridge(
        ...
        let payload: [String: Any] = [
            ...
            "event": [
                "sender": ["sender_id": ["open_id": "cnb-mac-companion"]],
                "message": [
