...
def test_cnb_runtime_paths_count_as_board_churn():
    # Regression: previously `.cnb/board.db` was misclassified because the
    # prefix list required `.cnb/board/` (trailing slash).
    for path in (".cnb/board.db", ".cnb/sessions/musk.md", ".claudes/board.db"):
        bucket, _ = classify_path(path, "M")
        assert bucket == "board/runtime churn", path
