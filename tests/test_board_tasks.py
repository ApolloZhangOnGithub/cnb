...
class TestTaskAdd:
    ...
    def test_first_task_becomes_active(self, db_conn):
        ...
        db_conn.execute(
            "INSERT INTO tasks(session, description, status, priority) VALUES (?, ?, 'active', 0)",
            ("alice", "implement feature X"),
        )
        db_conn.commit()

        row = db_conn.execute("SELECT status FROM tasks WHERE session='alice'").fetchone()
    ...
    def test_second_task_stays_pending(self, db_conn):
        ...
        db_conn.execute(
            "INSERT INTO tasks(session, description, status, priority) VALUES (?, ?, 'pending', 0)",
            ("alice", "task 2"),
        )
        db_conn.commit()

        rows = db_conn.execute("SELECT description, status FROM tasks WHERE session='alice' ORDER BY id").fetchall()
        assert len(rows) == 2
    ...
    def test_task_add_with_priority(self, db_conn):
        ...
        row = db_conn.execute(
            "SELECT description FROM tasks WHERE session='alice' ORDER BY priority DESC LIMIT 1"
        ).fetchone()
    ...
    def test_task_add_to_another_session(self, db_conn):
        ...
        row = db_conn.execute("SELECT session, description FROM tasks WHERE session='bob'").fetchone()
    ...
    def test_negative_priority(self, db_conn):
        ...
        db_conn.execute(
            "INSERT INTO tasks(session, description, status, priority) VALUES (?, ?, 'pending', -1)",
            ("alice", "low priority"),
        )
        db_conn.execute(
            "INSERT INTO tasks(session, description, status, priority) VALUES (?, ?, 'pending', 1)",
        ...
        rows = db_conn.execute(
            "SELECT description, priority FROM tasks WHERE session='alice' ORDER BY priority DESC"
        ).fetchall()
...
class TestTaskDone:
    ...
    def test_task_done_marks_as_done(self, db_conn):
        ...
        row = db_conn.execute("SELECT status, done_at FROM tasks WHERE id=?", (task_id,)).fetchone()
    ...
    def test_task_done_promotes_next_pending(self, db_conn):
        ...
        if active_count == 0:
            next_id = db_conn.execute(
                "SELECT id FROM tasks WHERE session='alice' AND status='pending' ORDER BY priority DESC, id ASC LIMIT 1"
            ).fetchone()
            if next_id:
                db_conn.execute(
        ...
        active = db_conn.execute("SELECT description FROM tasks WHERE session='alice' AND status='active'").fetchone()
    ...
    def test_task_done_no_pending_leaves_empty(self, db_conn):
        ...
        active = db_conn.execute("SELECT id FROM tasks WHERE session='alice' AND status='active'").fetchone()
    ...
    def test_already_done_task_noop(self, db_conn):
        ...
        now = ts()
        cur = db_conn.execute(
            "INSERT INTO tasks(session, description, status, priority, done_at) VALUES (?, ?, 'done', 0, ?)",
            ("alice", "old task", now),
        )
        task_id = cur.lastrowid
        db_conn.commit()
...
class TestTaskOwnership:
    ...
    def test_wrong_session_cannot_complete_others_task(self, db_conn):
        ...
        row = db_conn.execute("SELECT session FROM tasks WHERE id=?", (task_id,)).fetchone()
        assert row["session"] == "bob"
...
class TestTaskList:
    ...
    def test_task_list_all_sessions(self, db_conn):
        ...
        rows = db_conn.execute(
            "SELECT session, description FROM tasks WHERE status != 'done' ORDER BY session"
        ).fetchall()
        assert len(rows) == 2
    ...
    def test_task_list_include_done(self, db_conn):
        ...
        rows = db_conn.execute("SELECT description, status FROM tasks WHERE session='alice'").fetchall()
        assert len(rows) == 2
    ...
    def test_task_ordering_by_priority_and_id(self, db_conn):
        ...
        db_conn.execute(
            "INSERT INTO tasks(session, description, status, priority) VALUES (?, ?, 'pending', 10)",
            ("alice", "high prio"),
        )
        db_conn.execute(
            "INSERT INTO tasks(session, description, status, priority) VALUES (?, ?, 'pending', 0)",
        ...
        rows = db_conn.execute(
            "SELECT description FROM tasks WHERE session='alice' ORDER BY priority DESC, id ASC"
        ).fetchall()
